#目的:建立從csv、json、xml檔案格式中提取資料的自動化function，並資料自動轉換儲存成csv檔案，同時建立資料提取、轉換、載入的執行日誌
...
def extract_xml_file(filename):
    ...
    for car in xml_root:
        ...
        xml_df = pd.concat(
            [xml_df,
             pd.DataFrame([{'car_model':car_model,'year_of_manufacture':year_of_manufacture,'price':price,'fuel':fuel}])],
             ignore_index=True)
    return xml_df
...
def extract():
    ...
    for csvfile in glob.glob('*.csv'): #找到csv檔案並套用csv檔案資料提取的function
        all_data_df = pd.concat(
            [all_data_df, 
             pd.DataFrame(extract_csv_file(csvfile))],
             ignore_index = True)
    for jsonfile in glob.glob('*.json'): #找到json檔案並套用json檔案資料提取的function
        all_data_df = pd.concat(
            [all_data_df, 
             pd.DataFrame(extract_json_file(jsonfile))],
             ignore_index=True)
    for xmlfile in glob.glob('*.xml'): #找到xml檔案並套用xml檔案資料提取的function
        all_data_df = pd.concat(
            [all_data_df, 
             pd.DataFrame(extract_xml_file(xmlfile))],
             ignore_index=True)
    return all_data_df
